...
public class ShowProfile :  ApplicationCommandModule<ApplicationCommandContext>
{
    ...
    {
        if (!ulong.TryParse(strUserId, out ulong userId)) throw new BusinessException("Incorrect ID");

        User user = await Context.Client.Rest.GetUserAsync(userId) ?? throw new BusinessException("User not found");
